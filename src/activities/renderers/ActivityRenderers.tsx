...
export function SentenceBuildRenderer(props: BaseProps & { prompt: string; tokens: readonly string[]; answer: string }) {
  ...
  const retry = () => { setChosen([]); setResult(undefined); };
  return <div className="activity-renderer sentence-builder"><h3>{props.prompt}</h3><div className="sentence-builder__answer" aria-label="Your sentence">{chosen.length ? chosen.map((token, index) => <button key={`${token}-${index}`} disabled={correct} onClick={() => setChosen(current => current.filter((_, i) => i !== index))}>{token}</button>) : <span>Choose words below…</span>}</div><div className="sentence-builder__tokens">{available.map((token, index) => <Button key={`${token}-${index}`} variant="secondary" disabled={correct} onClick={() => setChosen(current => [...current, token])}>{token}</Button>)}</div><div className="lesson-inline-actions"><Button disabled={!chosen.length || Boolean(result)} onClick={() => void submit()}>Check sentence</Button>{result && !correct && <Button variant="secondary" onClick={retry}>Try again</Button>}</div>{result && <Feedback title={correct ? "Well built" : "Try again"} tone={correct ? "success" : "error"}>{correct ? props.answer : `Model: ${props.answer}`}</Feedback>}</div>;
}
