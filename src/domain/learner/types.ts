...
export interface AttemptHistorySummary { itemId: string; attemptCount: number; lastPracticedAt: string }
...
export interface LessonProgress { lessonId: string; learnerId: string; status: "in_progress" | "completed"; attemptCount: number; correctCount: number; currentStep: number; furthestStep: number; completedAt?: string; updatedAt: string; recalculationRequired: boolean }
